...
from models import MiniHabit, MiniHabitUpdate
...
async def create_habit(habit: MiniHabit, user_id: str = Depends(get_current_user_id)):
...
async def update_habit(id: str, habit: MiniHabitUpdate, user_id: str = Depends(get_current_user_id)):
    update_data = habit.dict(by_alias=True, exclude={"id", "user_id"}, exclude_unset=True)
    update_result = await habits_collection.update_one({"_id": ObjectId(id), "user_id": user_id}, {"$set": update_data})
    if update_result.modified_count == 1:
        if (updated_habit := await habits_collection.find_one({"_id": ObjectId(id), "user_id": user_id})) is not None:
            updated_habit["_id"] = str(updated_habit["_id"])
            return updated_habit
    if (existing_habit := await habits_collection.find_one({"_id": ObjectId(id), "user_id": user_id})) is not None:
...
async def delete_habit(id: str, user_id: str = Depends(get_current_user_id)):
    delete_result = await habits_collection.delete_one({"_id": ObjectId(id), "user_id": user_id})
